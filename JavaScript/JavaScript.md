...
1. 使用 `for...in` 遍历对象的属性
    ...
    // 遍历对象的非方法属性
    for (let key in circle) {
    	if (typeof circle[key] !== "function") console.log(key, circle[key]); // 输出: 10, function draw()
    }
    ```
...
>         ```
>
> 1. 每个函数在创建时，JavaScript 自动为其创建一个 `prototype` 对象。这个 `prototype` 对象上有一个默认的 `constructor` 属性，指向创建它的函数本身。
