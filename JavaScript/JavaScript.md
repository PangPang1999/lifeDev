...
- 内存分配：当我们创建一个对象或变量时，JavaScript 引擎会自动分配内存。例如，声明一个对象时，内存会在创建对象的同时被分配。
...
- 内存回收：在传统的编程语言中，程序员需要显式地释放不再使用的内存。在 JavaScript 中，**垃圾回收机制** 会自动完成内存的回收。当对象不再被任何变量引用时，它会被垃圾回收器标记为可回收，然后释放内存。
...
- 优点：
  ...
  - 简化开发：JavaScript 的垃圾回收机制使开发者不必担心内存管理细节，极大地简化了开发过程，特别是对于大型项目来说。

  - 减少内存泄漏风险：虽然垃圾回收器不能完美处理所有情况，但它减少了开发者因为忘记释放内存而导致的内存泄漏的风险。

- 限制：

  - 不确定性：垃圾回收器的执行时间和频率是不确定的。你无法控制何时垃圾回收会发生，因此无法保证内存回收的精确时机。
...
- 避免不必要的全局变量：全局变量可能会一直存在，导致内存无法被回收。
- 清除事件监听器和定时器：如果不再使用事件监听器或定时器，应该清除它们，否则可能会导致内存泄漏。
- 避免循环引用：当两个对象互相引用时，它们可能无法被垃圾回收器回收，从而导致内存泄漏。
