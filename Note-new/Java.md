...
1. 构造函数定义与作用
    ...
    - 定义：与类同名的特殊方法，无返回值类型，创建对象时自动执行。
    - 作用：明确地初始化对象字段，确保对象初始状态始终有效。
...
2. 默认构造函数
    ...
    - 定义格式：
        ...
        ```
    - 特性：
        - 当未显式定义构造函数时，由编译器自动生成，隐式存在。
        - 若定义了任何显式构造函数（含自定义构造函数），默认构造函数自动失效。
    - 作用：
        - 初始化对象字段为默认值：
            - 数值类型（`int`、`float`、`double`）：`0`
            - 布尔类型（`boolean`）：`false`
            - 引用类型（`Object`类型）：`null`
        - 隐式调用父类的无参构造函数（涉及继承关系，后续讲解）。
    - 补充说明：
        - 即便构造函数未显式定义任何初始化操作，JVM 也会保证对象各字段具有确定且安全的默认状态（不建议依赖默认值，应明确初始化）。
...
3. 自定义构造函数（Parameterized Constructor，推荐）
    ...
    - 特性：
        - 接受外部参数，显式初始化对象状态。
        - 一旦定义自定义构造函数，编译器不再自动创建默认构造函数。
        - 父类构造函数的调用责任隐式转移到自定义构造函数（涉及继承关系，后续讲解）。
    - 字段初始化方式：
        - 直接赋值（简单但不安全）：
            - 直接赋值存在风险，未进行参数合法性检查。
        - 通过 Setter 方法赋值（推荐）：
            - 可进行参数合法性校验，防止对象进入无效状态。
            - 提升代码安全性、可维护性与健壮性。

4. 构造函数设计原则

    - 一致性原则：
        - 使用构造函数一次性完成对象初始状态的所有必要设置，避免创建后额外的状态修改。
    - 安全性原则：
        - 强制初始化对象为有效状态，防止出现模糊、不安全的中间状态。
    - 封装与抽象原则：
        - 通过构造函数实现对象状态的封装，隐藏细节，仅暴露明确、安全的初始化方式。
