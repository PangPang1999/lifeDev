...
1. 云平台概览与选型
    ...
    - 常见平台：AWS、Heroku、DigitalOcean、Railway。
    - Railway 优势：
        - 简单易用，界面友好。
        - 支持多种数据库与一键部署。
        - 免费额度适合学习与原型开发。
...
2. railway 使用步骤（部署 mysql 举例）

    - 官方网址
        - http://railway.com/
    - 操作流程
        1. 使用 GitHub 账号注册或登录 Railway。
        2. 新建项目，在服务市场中搜索并一键部署 MySQL。
        3. 部署完成后，仅创建空数据库实例。表结构可通过应用启动时自动迁移工具（如 Flyway）自动初始化，无需手动建表。
    - 面板示意
        - Deployment：查看部署状态与历史版本。
        - Data：可视化数据库表与数据。
        - Backups：数据库自动备份与恢复。
        - Variables：集中管理连接密钥与环境变量。
        - Metrics：实时监控 CPU、内存与流量消耗。
        - Setting：配置总览。
