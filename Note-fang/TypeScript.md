...
1. 对象结构推断与显式注解
    ...
    - 显式注解限制对象结构（即有哪些属性、属性类型如何等），强制初始化必填属性
...
> 简述：通过使用 TypeScript 的类型别名（type alias），我们能集中定义对象的结构，避免重复编写相同的代码，保证数据一致性并提升代码的可读性。
...
1. 可选属性访问
    ...
    ```typescript
    // 假设 customer 类型为 { birthday?: Date }，且可能为 null/undefined
    console.log(customer?.birthday);
    ```

    - (仅在 customer 存在时访问 birthday，否则返回 undefined)

2. 可选元素访问与调用

    ```typescript
    // 访问可能为空的 customers 数组的第一个元素
    console.log(customers?.[0]);

    // log 可能为 null/undefined，只有在 log 存在时调用
    log?.("Message");
    ```
